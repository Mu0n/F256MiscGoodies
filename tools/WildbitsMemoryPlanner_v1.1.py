#!/usr/bin/env python3
"""
Wildbits / Foenix F256 — Memory Map Planner v1.1
============================================================
...
#
# "sprites" and "tilesets" are multi-size categories (`multi=True`, `dims`
# lists the allowed per-item dimensions): placing one asks for a size AND
# a count via MultiSizePlacementDialog, and the byte size is computed as
# dim * dim * count at that point, rather than being fixed up front.
RESOURCE_TYPES = {
    "bitmap":        dict(label="Bitmap 320x240",  size=320 * 240, color="#4C72B0"),
    "bitmap70Hz":    dict(label="Bitmap 320x200",  size=320 * 200, color="#6D95C3"),
    "sprites":       dict(label="Sprites", size=16 * 16 * 1, color="#DD8452",
                           multi=True, dims=(8, 16, 24, 32), default_count=1,
                           unit_label="sprite"),
    "tilesets":      dict(label="Tilesets", size=16 * 16 * 256, color="#00AAB8",
                           multi=True, dims=(8, 16), default_count=256,
                           unit_label="tile"),
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Wildbits — Memory Map Planner v1.1")
    ...
    def _build_toolbar(self):
        ...
        self.size_entry = ttk.Entry(bar, textvariable=self.size_var, width=8,
                                     font=("TkDefaultFont", F_SMALL))
        self.size_entry.pack(side="left", padx=(0, 8))
    ...
    def on_physical_click(self, page, subcell):
        ...
        if tool == "add":
            ...
            if info.get("multi"):
                def confirm_multi(addr, size_bytes):
                ...
                MultiSizePlacementDialog(self, "Place object", info["label"],
                                          info["dims"], info.get("default_count", 1),
                                          info.get("unit_label", "item"),
                                          start_addr, confirm_multi)
                return
...
class MultiSizePlacementDialog(tk.Toplevel):
    """Used for merged multi-size categories ('Sprites', 'Tilesets'): asks
    for a per-item dimension (e.g. 8x8/16x16/24x24/32x32), a count, and the
    start address, then computes the total byte size as dim * dim * count
    right before placing -- rather than that size being fixed up front."""

    def __init__(self, app, title, label_text, dims, default_count, unit_label,
                 suggested_start, on_confirm):
        super().__init__(app)
        self.app = app
        self.on_confirm = on_confirm
        ...
        size_row.pack(padx=14, pady=(4, 4), fill="x")
        ttk.Label(size_row, text="Size:", font=("TkDefaultFont", F_SMALL)).pack(side="left")
        self.dim_var = tk.IntVar(value=dims[0])
        for d in dims:
            ttk.Radiobutton(size_row, text=f"{d}x{d}", value=d, variable=self.dim_var,
                             command=self._update_total).pack(side="left", padx=(6, 0))

        count_row = ttk.Frame(self)
        count_row.pack(padx=14, pady=(8, 4), fill="x")
        ttk.Label(count_row, text=f"How many {unit_label}s:",
                  font=("TkDefaultFont", F_SMALL)).pack(side="left")
        self.count_var = tk.StringVar(value=str(default_count))
        count_entry = ttk.Entry(count_row, textvariable=self.count_var, width=8,
                                 font=("TkDefaultFont", F_SMALL))
        count_entry.pack(side="left", padx=6)
        count_entry.bind("<KeyRelease>", lambda e: self._update_total())

        self.total_var = tk.StringVar(value="")
        ttk.Label(self, textvariable=self.total_var, font=("TkDefaultFont", F_SMALL),
                  foreground="#555555").pack(padx=14, pady=(2, 0), anchor="w")

        addr_row = ttk.Frame(self)
        addr_row.pack(padx=14, pady=(10, 4), fill="x")
        ttk.Label(addr_row, text="Start address:", font=("TkDefaultFont", F_SMALL)).pack(side="left")
        self.addr_var = tk.StringVar(value=fmt_addr(suggested_start, app.addr_mode))
        entry = ttk.Entry(addr_row, textvariable=self.addr_var, width=14,
                           font=("TkDefaultFont", F_SMALL))
        entry.pack(side="left", padx=6)
        entry.focus_set()
        entry.select_range(0, "end")
    ...
    def _update_total(self):
        d = self.dim_var.get()
        n = self._parsed_count()
        if n is None:
            self.total_var.set("Enter a positive whole number of "
                                f"{self.unit_label}s.")
            return
        self.total_var.set(f"= {d} x {d} x {n} {self.unit_label}s = {d * d * n} bytes")
    ...
    def _confirm(self):
        ...
        if n is None:
            messagebox.showwarning(
                "Invalid count",
                f"Enter a positive whole number of {self.unit_label}s.",
                parent=self)
            return
        size_bytes = d * d * n
        ...
        try:
            ...
            messagebox.showwarning("Invalid address",
                                    ...
                                    parent=self)
            return
        self.on_confirm(addr, size_bytes)
        self.destroy()
